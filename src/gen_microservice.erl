...
%%%         {stop, Reason}
%%%
%%%   monolithic_call(Msg, {From, Tag}, State)
%%%
%%%    ==> {reply, Reply, State}
...
-callback monolithic_call(Request :: term(), From :: {pid(), Tag :: term()},
                      State :: term()) ->
...
-callback modern_async(Request :: term(), State :: term()) ->
    {noreply, NewState :: term()} |
    {noreply, NewState :: term(), timeout() | hibernate} |
    {stop, Reason :: term(), NewState :: term()}.
...
%%
%% Description: Makes an existing process into a gen_microservice.
%%              The calling process will enter the gen_microservice receive
%%              loop and become a gen_microservice process.
...
try_dispatch({'$gen_cast', Msg}, Mod, State) ->
    try_dispatch(Mod, modern_async, Msg, State);
...
try_monolithic_call(Mod, Msg, From, State) ->
    try
	{ok, Mod:monolithic_call(Msg, From, State)}
    catch
	throw:R ->
	    {ok, R};
	error:R ->
...
handle_msg({'$gen_call', From, Msg}, Parent, Name, State, Mod) ->
    Result = try_monolithic_call(Mod, Msg, From, State),
    case Result of
